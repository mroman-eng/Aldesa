...
variable "gold_dataset_id" {
  description = "Gold dataset id that Looker Studio users can query."
  type        = string

  validation {
    condition     = can(regex("^[A-Za-z_][A-Za-z0-9_]*$", var.gold_dataset_id)) && length(var.gold_dataset_id) <= 1024
    error_message = "gold_dataset_id must follow BigQuery dataset naming rules."
  }
}
