...
variable "bronze_parquet_bucket" {
  ...
  validation {
    condition = (
      try(var.bronze_parquet_bucket.public_access_prevention, null) == null ||
      contains(["enforced", "inherited"], lower(var.bronze_parquet_bucket.public_access_prevention))
    )
    error_message = "bronze_parquet_bucket.public_access_prevention must be either enforced or inherited."
  }

  validation {
    condition     = try(var.bronze_parquet_bucket.delete_noncurrent_versions_after_days, null) == null || var.bronze_parquet_bucket.delete_noncurrent_versions_after_days > 0
    error_message = "bronze_parquet_bucket.delete_noncurrent_versions_after_days must be greater than 0 when set."
  }
}
