...
bootstrap_remote_state = {
  ...
  bootstrap_prefix = "shared/00-bootstrap"
}

orchestration_remote_state = {
  bucket               = "data-buildtrack-dev-tfstate-europe-west1"
  orchestration_prefix = "pre/30-orchestration"
}
...
cloudbuild = {
  ...
  dags_sync_pipeline = {
    ...
    display_name               = "Cloud Build DAG sync (PRE)"
  ...
  triggers = [
    ...
    },
    {
      ...
      included_files = [
        ...
        "terraform/envs/pre/20-storage-bq/**",
        "terraform/envs/pre/30-orchestration/**",
        "terraform/envs/pre/40-governance/**",
        "terraform/envs/pre/50-bi/**",
        "terraform/envs/pre/60-cicd/**",
        "functions/**",
    ...
    },
    {
      ...
      description         = "PR validation for DEV Terraform changes - Always-on"
      event               = "PULL_REQUEST"
      disabled            = false
      branch_regex        = "^develop$"
      comment_control     = "COMMENTS_DISABLED"
    ...
    # Push to develop (deploy PRE with a single ordered Terraform pipeline)
    {
      ...
      description         = "Apply Terraform components in dependency order in PRE on push to develop."
      event               = "PUSH"
      disabled            = true
      branch_regex        = "^develop$"
    ...
    {
      name                = "cb-pre-dags-sync"
      description         = "Sync repository dags/ to Composer DAG bucket in PRE on push to develop."
      event               = "PUSH"
      disabled            = true
      branch_regex        = "^develop$"
    ...
    {
      name                = "cb-dev-tf-apply"
      description         = "Reconcile DEV stack on push to develop when DEV Terraform changes."
      event               = "PUSH"
      disabled            = false
      branch_regex        = "^develop$"
      filename            = "cloudbuild/push-dev-terraform-apply.yaml"
      service_account_ref = "terraform"
      substitutions = {
        _ENV = "dev"
      }
